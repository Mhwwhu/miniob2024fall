...
public:
	 ...
	 */
	RC insert_entry(vector<pair<const char*, int>> user_keys, const RID* rid);
	 ...
	 * @note 这里假设user_key的内存大小与attr_length 一致
	 */
	RC delete_entry(vector<pair<const char*, int>> user_keys, const RID* rid);
...
private:
	common::MemPoolItem::item_unique_ptr make_key(vector<pair<const char*, int>> user_keys, const RID& rid);
...
public:
	...
	/**
	 * @brief 扫描指定范围的数据
	 * @param left_user_keys 扫描范围的左边界，如果是empty，则没有左边界

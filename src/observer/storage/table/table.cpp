...
RC Table::drop(Db* db)
{
	...
	RC rc = sync();
	if (OB_FAIL(rc)) {
		LOG_WARN("Failed to flush data when dropping data %s", name());
		return rc;
	}
	do {
		//删除表元数据
		string path = table_meta_file(base_dir_.c_str(), name());
		filesystem::copy_file(path, path + ".bak");
		remove_list.push_back(path);
		if (unlink(path.c_str()) != 0) {
			LOG_WARN("Failed to remove meta file %s", path.c_str());
			break;
		}
		//删除数据文件
		path = table_data_file(base_dir_.c_str(), name());
		filesystem::copy_file(path, path + ".bak");
		remove_list.push_back(path);
		if (unlink(path.c_str()) != 0) {
			LOG_WARN("Failed to remove data file %s", path.c_str());
			break;
		}
		//删除索引文件
		int index_num = table_meta_.index_num();
		for (int i = 0; i < index_num; i++) {
			string index_name = table_meta_.index(i)->name();
			path = table_index_file(base_dir_.c_str(), name(), index_name.c_str());
			filesystem::copy_file(path, path + ".bak");
			remove_list.push_back(path);
			if (unlink(path.c_str()) != 0) {
				LOG_WARN("Failed to remove index file %s", path.c_str());
				break;
			}
		}
		//删除备份文件
		for (string file : remove_list) {
			filesystem::remove(file + ".bak");
		}
		return rc;
	} while (false);
	// 恢复删除掉的文件
	for (string file : remove_list) {
		filesystem::rename(file + ".bak", file);
	}

...
RC CreateIndexStmt::create(Db* db, const CreateIndexSqlNode& create_index, Stmt*& stmt)
{
	...
	if (is_blank(table_name) || is_blank(create_index.index_name.c_str())) {
		LOG_WARN("invalid argument. db=%p, table_name=%p, index_name=%s",
	...
	for (auto attr_name : create_index.attribute_names) {
		if (is_blank(attr_name.c_str())) {
			LOG_WARN("invalid argument. db=%p, table_name=%p, index_name=%s, attr_name=%s",
	...
	stmt = new CreateIndexStmt(table, field_meta_list, create_index.index_name, create_index.unique);
	return RC::SUCCESS;
}
